...
test('Project view tab navigation', {tag: ['@LPD-77908']}, async ({apiHelpers, editTaskPage, page, projectPage, projectsPage}) => {
    ...
    try {
        await test.step('Access details tab and create a task using the "new task" button', async () => {
            ...
            await expect(projectPage.detailsTab).toHaveAttribute('aria-selected', 'true');
        });
        ...
        await test.step('Access tasks tab and create a task using the "new" button', async () => {
            ...
            await expect(projectPage.tasksTab).toHaveAttribute('aria-selected', 'true');
        });
