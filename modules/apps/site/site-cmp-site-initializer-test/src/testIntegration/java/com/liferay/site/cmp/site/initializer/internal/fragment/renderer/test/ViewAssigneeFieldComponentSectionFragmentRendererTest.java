...
public class ViewAssigneeFieldComponentSectionFragmentRendererTest
	...
	public void testGetProps() throws Exception {
		...
		_taskObjectEntry = _objectEntryLocalService.partialUpdateObjectEntry(
			...
			HashMapBuilder.<String, Serializable>put(
				"assignTo",
				HashMapBuilder.put(
					"classNameId", String.valueOf(className.getClassNameId())
				).put(
					"classPK", String.valueOf(role.getRoleId())
				).build()
			).build(),
			ServiceContextTestUtil.getServiceContext());

		_refreshLayoutDisplayPageObjectProvider(_taskObjectEntry);

		props = getProps();

		JSONObject jsonObject = _jsonFactory.createJSONObject(props);
		...
		_taskObjectEntry = _objectEntryLocalService.partialUpdateObjectEntry(
			...
			HashMapBuilder.<String, Serializable>put(
				...
				HashMapBuilder.put(
					...
					"classPK", String.valueOf(user.getUserId())
				).build()
			).build(),
			ServiceContextTestUtil.getServiceContext());
		...
		jsonObject = _jsonFactory.createJSONObject(props);
	...
	private void _refreshLayoutDisplayPageObjectProvider(
