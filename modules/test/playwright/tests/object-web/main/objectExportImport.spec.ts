...
test('Field values persist for object entries with 100 fields after export and import',
...
test('Can cancel importing an object',
...
test('Can clear the JSON file on the import dialog',
	async ({apiHelpers, page, viewObjectDefinitionsPage}) => {
		const objectDefinition =
			await apiHelpers.objectAdmin.postRandomObjectDefinition({
				status: {code: 0},
...
test('Can export an object with Actions',
	async ({apiHelpers, page, viewObjectDefinitionsPage}) => {
		const objectDefinition =
			await apiHelpers.objectAdmin.postRandomObjectDefinition({
				status: {code: 0},
...
test('Can export an object with Aggregation field',
	async ({
		apiHelpers,
		objectFieldsPage,
		page,
...
test('Can import and export Custom Views structure',
	async ({apiHelpers, page, viewObjectDefinitionsPage}) => {
		const objectFields = generateObjectFields({
...
test('Can import and export State Manager structure',
	async ({apiHelpers, page, viewObjectDefinitionsPage}) => {
		test.setTimeout(300000);
		const listTypeDefinition =
			await apiHelpers.listTypeAdmin.postRandomListTypeDefinition();
...
test('Can import and export Validation structure',
	async ({apiHelpers, page, viewObjectDefinitionsPage}) => {
		const objectFields = generateObjectFields({
			objectFieldBusinessTypes: ['Text'],
		});
...
test('Can import data structure to custom objects',
	async ({apiHelpers, page, viewObjectDefinitionsPage}) => {
		const objectDefinition =
			await apiHelpers.objectAdmin.postRandomObjectDefinition({
				status: {code: 0},
...
test('Can import and maintain Fields after importing an Object',
	async ({apiHelpers, page, viewObjectDefinitionsPage}) => {
		const objectFields = generateObjectFields({
			objectFieldBusinessTypes: ['Text'],
		});
...
test('Can import and maintain Layouts after importing an Object',
	async ({
		apiHelpers,
		objectLayoutsPage,
		page,
...
test('Can import an object with Actions',
	async ({apiHelpers, page, viewObjectDefinitionsPage}) => {
		const objectDefinition =
			await apiHelpers.objectAdmin.postRandomObjectDefinition({
				status: {code: 0},
...
test('Can import the same object more than once',
	async ({apiHelpers, page, viewObjectDefinitionsPage}) => {
		const objectDefinition =
			await apiHelpers.objectAdmin.postRandomObjectDefinition({
				status: {code: 0},
...
test('Can import and maintain Scope after importing an Object',
	async ({apiHelpers, page, viewObjectDefinitionsPage}) => {
		const objectDefinition =
			await apiHelpers.objectAdmin.postRandomObjectDefinition({
				scope: 'company',
...
test('Imported custom object is created with Draft status',
	async ({apiHelpers, page, viewObjectDefinitionsPage}) => {
		const objectDefinition =
			await apiHelpers.objectAdmin.postRandomObjectDefinition({
				status: {code: 2},
